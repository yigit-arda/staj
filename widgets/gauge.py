...
class GaugeWidget(QWidget):
    """Tur sayacı (odometer) tarzı, 360 derece dönen ibreli gösterge.

    Değerin TAM kısmı ortada "tur sayısı" olarak yazılır.
    Değerin ONDALIK kısmı, ibrenin 360 derece içinde nerede duracağını belirler:
    örn. değer 3.75 ise -> tur sayısı 3, ibre saat 12'den başlayıp %75 dönmüş
    konumda (yani saat 9 yönünde) durur.

    set_range(min, max) ile değer sınırlanır (opsiyonel kullanım);
    set_value(value) her zaman gösterge değerini günceller.
    """
    ...
    def paintEvent(self, event):
        ...
        # Tam kısım (tur sayısı) ve ondalık kısım (ibrenin açısı).
        # floor kullanıyoruz ki negatif değerlerde de ondalık kısım hep [0,1) aralığında kalsın.
