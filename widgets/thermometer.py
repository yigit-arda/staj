...
class VerticalThermometer(QWidget):
    """A custom widget that displays a vertical thermometer with a digital readout.

    Features a graphical mercury tube, temperature scale marks, and a digital 
    display at the bottom showing the current temperature in Celsius.

    Attributes:
        current_temp (float): The current temperature value displayed.
        min_temp (float): The minimum temperature bound.
        max_temp (float): The maximum temperature bound.
        digital_font_family (str): The font family used for the digital text display.
    """
     
    def __init__(self, parent=None):
        super().__init__(parent)
        """Initializes the VerticalThermometer widget.
    ...
    def set_range(self, min_val, max_val):
        """Sets the minimum and maximum temperature range for the scale.

        Args:
            min_val (float or int): The minimum temperature value.
            max_val (float or int): The maximum temperature value.
        """
    ...
    def set_temperature(self, value):
        """Updates the current temperature value and triggers a widget repaint.

        Args:
            value (float or int): The new temperature value to display.
        """
        self.current_temp = max(self.min_temp, min(float(value), self.max_temp))
        self.update()

    def paintEvent(self, event):
        """Handles the paint event to draw the thermometer, scale marks, and text.

        Calculates dynamic drawing bounds for the glass tube, maps the current 
        temperature to a physical pixel height, and renders the red liquid column 
        along with the digital text display.
        ...
        # Calculate bounding coordinates for the thermometer tube and bulb
        tube_w = 16
        tube_h = height - 120 
        ...
        # Define the vertical span available for the temperature scale
        ...
        # Draw the scale marks and numerical labels using a 10-degree step
        ...
        # Render the outer glass casing of the tube and the bulb
        ...
        # Calculate the dynamic height of the red liquid column based on current temperature
        ...
        # Render the inner liquid column and bulb
        ...
        # Add a light reflection effect on the bottom bulb
        painter.setBrush(QBrush(QColor(255, 255, 255, 50)))
        painter.drawEllipse(QPointF(bulb_cx - 5, bulb_cy - 5), 6, 6)

       # Draw the digital numeric display below the thermometer structure
